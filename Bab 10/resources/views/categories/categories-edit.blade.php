...
<div class="form-login">
  <form action="{{ url('/category/update/' . $category->id_categories) }}" method="post" enctype="multipart/form-data">
    ...
    <input class="input" type="text" name="region" id="rekion" placeholder="region"
      value="{{ old('nama', $category->region) }}" />
    @error('region')
    <p style="font-size: 10px; color: red">{{ $message }}</p>
    @enderror
    <label for="categories">Kategori</label>
    <input class="input" type="text" name="kategori" id="categories" placeholder="Categories"
